...
rules <- apriori(transactions_raw, parameter = list(supp = 0.005, conf = 0.4, minlen = 2, maxlen = 10)) #first rules using supp = 0.001 and
...
# Store rules in df
df_rules <- inspect(rules_sorted)
df_rules$rhs_cat <- NA
i <- NULL
for (i in nrow(df_rules)) {
  rhs <- as.character(df_rules[i, which(colnames(df_rules) == "rhs")])
  rhs_cat <- df_names[agrep(rhs, df_names$il_raw, ignore.case = TRUE, value = FALSE, max.distance = ), which(colnames(df_names) == "category")]
  df_rules[i, which(colnames(df_rules) == "rhs_cat")] <- rhs_cat
}
df$rhs_category <- NULL
