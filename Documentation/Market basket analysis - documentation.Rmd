...
rows_b2c_trans <- unique(c(twocpus, twoprints, twomons, tensize))

# Create seperate S4 for b2c and b2b
trans_b2c <- as(df_comparison[rows_b2c_trans, ] == 1, "transactions")
trans_b2b <- as(df_comparison[-c(twocpus, twomons, twoprints, tensize), ] == 1, "transactions")
```
...
rules_product <- apriori(transactions_raw, parameter = list(supp = 0.005, conf = 0.4, minlen = 2, maxlen = 10)) 
rules_category <- apriori(trans_category, parameter = list(supp = 0.015, conf = 0.75, minlen = 2, maxlen = 10))
rules_brand <- apriori(trans_brand, parameter = list(supp = 0.015, conf = 0.70, minlen = 2, maxlen = 10)) 
rules_b2c <- apriori(trans_b2c, parameter = list(supp = 0.015, conf = 0.75, minlen = 2, maxlen = 10)) 
rules_b2b <- apriori(trans_b2b, parameter = list(supp = 0.015, conf = 0.75, minlen = 2, maxlen = 10)) 

# Exclude redundant and sort rules
rules_sorted_product <- sort(rules_product[!is.redundant(rules_product)], by = "lift", decreasing = TRUE)
rules_sorted_category <- sort(rules_category[!is.redundant(rules_category)], by = "lift", decreasing = TRUE)
rules_sorted_brand <- sort(rules_brand[!is.redundant(rules_category)], by = "lift", decreasing = TRUE)
rules_sorted_b2c <- sort(rules_b2c[!is.redundant(rules_b2c)], by = "lift", decreasing = TRUE)
rules_sorted_b2b <- sort(rules_b2b[!is.redundant(rules_b2b)], by = "lift", decreasing = TRUE)
...
df_rules$rhs_cat <- NA
df_rules$rhs_brand <- NA
for (i in c(1:nrow(df_rules))) {
  a <- gsub("{", "", df_rules[i, which(colnames(df_rules) == "rhs")], fixed = T)
  a2<- gsub("}", "", a, fixed = T)
  ...
  df_rules[i, which(colnames(df_rules) == "rhs_cat")] <- b
  df_rules[i, which(colnames(df_rules) == "rhs_brand")] <- c
  i <- i + 1
}
